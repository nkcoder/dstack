...
Use your configured architect runners (defaults `claude-opus-5-thinking-high`, `claude-sonnet-5-thinking-high`). Claude Code can only spawn Claude-family models, so the diversity signal here comes from spreading runners across Claude's distinct model tiers rather than across vendors.
